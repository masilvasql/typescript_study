...
//parâmetro padrão
function contagemRegressiva(inicio: number = 10, fim: number = inicio - 5): void {
    console.log(inicio)
    while (inicio >= fim) {
        inicio--;
        console.log(inicio)
...
const usuarioID: string = "Suporte teste";
const notificacoes: string = '10'
const boasVindas: string = 'Boas vindas ' + usuarioID + ' Notificações ' + notificacoes;
const boasVindas2: string = `Boas vindas ${usuarioID} Notificações ${parseInt(notificacoes) > 9 ? '+9' : notificacoes}`;
...
//exercícios
//1)
var dobro = function (valor: number = 5) {
    return valor * 2
}
console.log(dobro(10))

//2)
var dizerOla = function (nome:string | undefined):void {
    if (nome === undefined) { 
        nome = "Pessoa" 
    }
